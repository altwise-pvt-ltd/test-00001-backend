// Business logic for subject allocations — the authority join that says
...
const SubjectAllocation = require('./subjectAllocation.model');
...
async function listSubjectAllocations(schoolId, filter = {}) {
  ...
  return SubjectAllocation.find(query)
...
async function createSubjectAllocation(schoolId, data, userId) {
  ...
  // classId is authoritative from the section. If the caller supplied one, it
  // must match — otherwise the request is internally inconsistent.
  if (data.classId && String(data.classId) !== String(section.classId)) {
    throw ApiError.badRequest('classId does not match the section\'s class');
  }
  ...
  if (existing) throw ApiError.conflict('This subject allocation already exists');

  return SubjectAllocation.create({ teacherId, subjectId, classId, sectionId, schoolId, createdBy: userId });
}

// Replace a teacher's WHOLE set of subject allocations (subject+section pairs)
...
async function syncTeacherAllocations(schoolId, teacherId, desired, userId) {
  const teacher = await User.findOne({ _id: teacherId, schoolId, deletedAt: null });
  if (!teacher || teacher.role !== USER_ROLES.TEACHER) {
  ...
  // every subject + section must belong to this school. We also stash each
  // section's classId on the pair so creates below can persist it (derived,
  // never trusted from the client).
  for (const pair of pairs) {
    const subject = await Subject.findOne({ _id: pair.subjectId, schoolId, deletedAt: null });
    if (!subject) throw ApiError.badRequest(`subjectId ${pair.subjectId} is not a subject in this school`);
    const section = await Section.findOne({ _id: pair.sectionId, schoolId, deletedAt: null });
    if (!section) throw ApiError.badRequest(`sectionId ${pair.sectionId} is not a section in this school`);
    pair.classId = section.classId;
  }

  // current rows for this teacher, INCLUDING soft-deleted (so we can revive)
  const existing = await SubjectAllocation.find({ teacherId, schoolId });
  ...
  for (const { subjectId, sectionId, classId } of pairs) {
    const doc = existingByKey.get(`${subjectId}|${sectionId}`);
    if (!doc) {
      await SubjectAllocation.create({ teacherId, subjectId, classId, sectionId, schoolId, createdBy: userId });
      created += 1;
...
async function deleteSubjectAllocation(id, schoolId, userId) {
  const doc = await SubjectAllocation.findOneAndUpdate(
    { _id: id, schoolId, deletedAt: null },
    { deletedAt: new Date(), updatedBy: userId },
    { new: true }
  );
  if (!doc) throw ApiError.notFound('Subject allocation not found');
  return doc;
}
